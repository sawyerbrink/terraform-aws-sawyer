...
######
#############################
# UI Domain Certificate
############################

resource "aws_acm_certificate" "ui-cert" {
  domain_name = "ui.${var.backendinfra-domain-name}"
  # subject_alternative_names = []
  validation_method = "DNS"

  tags = merge({ "Name" = "ui.${var.backendinfra-domain-name}" }, var.tags)
...
resource "aws_route53_record" "ui_cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.ui-cert.domain_validation_options : dvo.domain_name => {
      name    = dvo.resource_record_name
      record  = dvo.resource_record_value
      type    = dvo.resource_record_type
      zone_id = data.aws_route53_zone.primary.zone_id
...
resource "aws_acm_certificate_validation" "cert-2" {
  certificate_arn         = aws_acm_certificate.ui-cert.arn
